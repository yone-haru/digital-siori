...
## Account Settings 画面（4スクリーン）

設定画面のすべて：プロフィール・セキュリティ・Dangerの3セクションに論理分割。ダークモード固定（書籍詳細画面と同じ`#0F0D0A`）。

参照ファイル：
- `アカウント設定 Screens.html`（4アートボードのキャンバス）
- `screens-account.jsx`（コンポーネント実装）
- `exports/account-1-main.png` 〜 `account-4-delete-account.png`（プレビュー画像）

### Avatar カラーパレット

`AVATAR_COLORS` は書影パレットと同じ8色から（"色は本の表紙から生まれる"原則をアバターにも適用）。デフォルトのアバターはユーザー表示名のイニシャル1文字を白92%透過で表示し、背景色はユーザーIDのハッシュで決定論的に選ぶ。
...
**Top bar:** ← back / "ACCOUNT" ラベル

**Hero（中央寄せ・padding: 24px 28px 36px）:**
- Avatar 96px。右下に編集バッジ（30×30の `#F7F5EF` 円 + 黒カメラアイコン）を `position:absolute` で重ねる
- 表示名（Shippori Mincho 22px / weight:600 / color:rgba(255,255,255,0.92)）
- メール（Cormorant Garamond 13px / color:rgba(255,255,255,0.55)）

**Section: Profile**
- ラベル（"PROFILE" Zen Kaku 10px / letter-spacing:0.22em / color:white35）padding 0 28px 12px
- Row "アイコン" — 右側にミニアバター（28px）+ chevron。タップで A2 へ
- Row "表示名" — value 表示 + サブで「本棚やレビューに表示されます」。タップで A3 へ

**Section: Security**
- Row "メールアドレス" — value のみ表示、chevron なし（編集不可）
- Row "パスワードを変更" — サブで「最終更新 2026年 3月」。タップでパスワード変更フロー（別途設計）へ

**Danger zone**（ラベルなし、視覚的に分離するためのスペース32-36px）
- Row "ログアウト" — label color: `#C77B6F`、右端に LogoutIcon
- Row "アカウントを削除" — label color: `#C77B6F`、サブで「本棚や読書履歴も含めて完全に消えます」、右端に TrashIcon。タップで A4 へ

**Footer:**
- 区切り線 + version 表示「DIGITAL BOOKMARK · v 1.0.2」(Cormorant 11px / letter-spacing:0.2em / color:white25)

**Bottom Nav:** LIBRARY タブがアクティブ（設定画面は LIBRARY タブから入る想定）

### Screen A2 — アイコン変更（ボトムシート）

**トリガー:** メイン画面のアイコン行タップ

**レイアウト:**
- 背景: メイン画面を opacity 0.18 で残し、上に `rgba(0,0,0,0.55)` のバックドロップ
- ボトムシート: `position:absolute, left:0, right:0, bottom:0`, background `#16140F`, border-radius `16px 16px 0 0`, padding `12px 0 32px`, shadow `0 -16px 40px rgba(0,0,0,0.6)`
- 上部にドラッグハンドル（40×3px, white25, margin: 0 auto 22px）

**コンテンツ:**
1. ヘッダー: "CHANGE AVATAR" ラベル + 「アイコンを変更」（Shippori Mincho 20px / weight:500）, padding: 0 28px 16px
2. プレビュー: Avatar 104px, 中央寄せ, padding: 12px 0 32px
3. アクション行: 「写真から選ぶ」(CameraIcon + Zen Kaku 14px) — 上下に1px線, padding: 18px 28px
4. キャンセルボタン（outline, height:50px）

**動作:** 「写真から選ぶ」タップで OS のファイルピッカーを開く（`<input type="file" accept="image/*">`）。選択後、自動的にクロップ＆プレビュー → 保存（Supabase Storage）。

### Screen A3 — 表示名の編集

**トリガー:** メイン画面の表示名行タップ

**Top bar:** 「キャンセル」(left) / "DISPLAY NAME" ラベル / 「保存」(right, color:white92, weight:500)

**Body:**
- "NAME" ラベル padding: 40px 28px 0
- 入力フィールド:
  - Shippori Mincho 28px / weight:500 / color:white92 / letter-spacing:0.04em
  - border-bottom: 1.5px solid white92（フォーカス時）
  - 点滅キャレット（1.5×30px の `background:white92`, animation: `blink 1s steps(2) infinite`）
- メタ行: 「本棚やレビューに表示されます」(left) / 「4 / 20」(right, Cormorant 11px) — color: white35

**バリデーション:**
- 1〜20文字
- 空白のみは不可
- 「保存」タップで Supabase の `profiles.display_name` を更新

### Screen A4 — アカウント削除確認

**トリガー:** メイン画面の「アカウントを削除」行タップ

**Top bar:** ← back / "DELETE ACCOUNT" ラベル

**Body（padding: 28px 28px 0）:**
- 見出し「このアカウントを削除しますか？」(Shippori Mincho 24px / weight:500 / line-height:1.55)
- 本文（Zen Kaku 13px / color:white55 / line-height:1.9）: 「すべての本棚・読書履歴・読書時間の記録が、復元できない形で消えます。」
- 失うものリストカード（bg: `#1A1815`, border-radius:2, padding: 20px 22px）:
  - "WILL BE LOST" ラベル
  - 3行の値: 登録した本（24 冊）/ 読書セッション（87 件）/ 累計読書時間（127 h 42 m）
  - 各行は 1px white-line で区切る
  - 数字は Cormorant Garamond 18px / weight:300 / white92
  - 単位は Zen Kaku 11px / white55
- 締めの一文（Shippori Mincho 14px / color:white70 / line-height:1.9）: 「この操作は取り消せません。」

**Sticky action area**（border-top: 1px white-line, padding: 20px 28px 32px）:
- 「アカウントを削除する」: bg `#C77B6F`, color `#0F0D0A`, height:52px, Zen Kaku 13px / letter-spacing:0.18em
- 「キャンセル」: outline, height:52px

**動作:** Supabase でユーザーレコードを物理削除 → 全関連データもRLSカスケードで消える → ログイン画面へリダイレクト。
...
| Prop | 型 | 説明 |
|------|----|------|
| `label` | string | 主ラベル（Zen Kaku 14px） |
| `value` | string? | 右側の値。英数字なら Cormorant、和文なら Shippori |
| `sub` | string? | label の下に小さく（11px, white35） |
| `danger` | bool | label を `#C77B6F` 色に |
| `chevron` | bool | デフォルト true。表示のみの行では false |
| `rightIcon` | Node? | chevron の代わりに任意のアイコン |
...
| `アカウント設定 Screens.html` | アカウント設定の4画面（メイン / アイコン変更 / 表示名 / 削除確認） |
| `screens-account.jsx` | アカウント設定コンポーネント実装 |
| `exports/account-*.png` | アカウント画面の高解像度プレビュー（390×844） |
